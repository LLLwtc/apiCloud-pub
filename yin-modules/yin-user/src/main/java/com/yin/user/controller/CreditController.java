...
import com.yin.api.user.model.dto.credit.CreditAddRequest;
import com.yin.api.user.model.dto.credit.CreditEditRequest;
import com.yin.api.user.model.dto.credit.CreditQueryRequest;
import com.yin.api.user.model.dto.credit.CreditUpdateRequest;
import com.yin.api.user.model.entity.Credit;
import com.yin.api.user.model.entity.User;
import com.yin.api.user.model.enums.UserConstant;
import com.yin.common.annotation.AuthCheck;
...
import com.yin.common.constant.CommonConstant;
import com.yin.common.exception.BusinessException;
import com.yin.common.util.SqlUtils;
...
public class CreditController {
    ...
    public BaseResponse<Page<Credit>> listMyCreditByPage(@RequestBody CreditQueryRequest creditQueryRequest,HttpServletRequest request) {
    ...
    public BaseResponse<Boolean> editCredit(@RequestBody CreditEditRequest creditEditRequest,HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(oldCredit == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldCredit.getUserId().equals(loginUser.getId()) && !userService.isAdmin(request)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        boolean result = creditService.updateById(credit);
        return ResultUtils.success(result);
